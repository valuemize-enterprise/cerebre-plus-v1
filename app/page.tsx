    ...
    <div style={{ fontFamily:'system-ui,-apple-system,sans-serif', background:A, color:'#EBF2FC', overflowX:'hidden' }}>
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .cta-main:hover{background:linear-gradient(135deg,#F5B830,#FFD055);transform:translateY(-2px);box-shadow:0 14px 40px rgba(224,152,24,0.4)}
      ...
      <nav style={{position:'sticky',top:0,zIndex:100,background:'rgba(6,12,26,0.88)',backdropFilter:'blur(22px)',borderBottom:'1px solid rgba(255,255,255,0.07)'}}>
        <div style={{maxWidth:1200,margin:'0 auto',padding:'0 36px',height:66,display:'flex',alignItems:'center',justifyContent:'space-between'}}>
          ...
          <div className="nav-links" style={{display:'flex',gap:32}}>
            {[['Tools','#tools'],['How it works','#how'],['Pricing','#pricing'],['SME Club','#sme']].map(([l,h])=>(
          ...
          <div style={{display:'flex',gap:10}}>
            ...
            <Link href="/signup" className="cta-main" style={{padding:'8px 22px',borderRadius:8,fontSize:13.5,fontWeight:700,textDecoration:'none',display:'inline-block'}}>
              Start free →
            </Link>
          </div>
        </div>
        ...
        <div className="hero-wrap" style={{display:'flex',width:'100%'}}>
          ...
          <div className="hero-txt" style={{flex:'0 0 45%',display:'flex',flexDirection:'column',justifyContent:'center',padding:'64px 52px 64px 80px'}}>
            ...
            <div className="fade3" style={{display:'flex',gap:12,flexWrap:'wrap'}}>
              <Link href="/signup" className="cta-main" style={{display:'inline-flex',alignItems:'center',gap:8,padding:'15px 30px',borderRadius:12,fontWeight:800,fontSize:15,textDecoration:'none',boxShadow:'0 8px 28px rgba(224,152,24,0.3)'}}>
                Run your first tool free →
              </Link>
      ...
      <section id="how" style={{background:B,padding:'104px 36px'}}>
        <div style={{maxWidth:1100,margin:'0 auto'}}>
          ...
          <div className="steps" style={{display:'flex',position:'relative',gap:0}}>
            <div className="steps-line" style={{position:'absolute',top:52,left:'14%',right:'14%',height:1,background:'rgba(224,152,24,0.18)'}}/>
            {[
      ...
      <section id="pricing" style={{background:B,padding:'104px 36px'}}>
        <div style={{maxWidth:980,margin:'0 auto'}}>
          ...
          <div className="grid-price" style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:18}}>
            {[
              {name:'Free',price:'₦0',period:'always',desc:'Run your first tool. See what Cerebre Plus does for your business before spending a naira.',features:['1 free tool run on signup','Preview all 40+ tools','Full business profile setup','AI-powered suggestions'],cta:'Start free',href:'/signup',highlight:false},
              {name:'Starter',price:'₦7,500',period:'/month',desc:'For businesses that market consistently. Coins refresh every month.',features:['250 coins monthly','Full access to all 40+ tools','Design Studio (11 visual tools)','Content Calendar','Priority email support'],cta:'Get started',href:'/signup?plan=starter',highlight:true,badge:'Most popular'},
              {name:'Growth',price:'₦15,000',period:'/month',desc:'For fast-moving businesses and teams that run on content every day.',features:['600 coins monthly','Everything in Starter','SME Club access (weekly sessions)','Competitor Intelligence 2.0','Team workspace'],cta:'Go Growth',href:'/signup?plan=growth',highlight:false},
            ].map(({name,price,period,desc,features,cta,href,highlight,badge})=>(
              <div key={name} style={{background:highlight?`linear-gradient(160deg,rgba(224,152,24,0.1),rgba(6,12,26,0.85))`:A,border:highlight?'1.5px solid rgba(224,152,24,0.42)':'1px solid rgba(255,255,255,0.07)',borderRadius:20,padding:30,display:'flex',flexDirection:'column',position:'relative',boxShadow:highlight?'0 20px 56px rgba(224,152,24,0.14)':'none'}}>
                {badge&&<span style={{position:'absolute',top:-13,left:'50%',transform:'translateX(-50%)',fontSize:10.5,fontWeight:800,letterSpacing:'1.5px',padding:'4px 16px',borderRadius:20,background:'linear-gradient(135deg,#E09818,#F5B830)',color:'#060C1A',whiteSpace:'nowrap'}}>{badge}</span>}
                <p style={{fontSize:11.5,fontWeight:700,letterSpacing:'1.5px',textTransform:'uppercase',color:highlight?'#F5B830':'rgba(235,242,252,0.38)',marginBottom:16}}>{name}</p>
                <div style={{marginBottom:10}}>
                  <span style={{fontFamily:"'Georgia',serif",fontSize:40,fontWeight:900,color:'#EBF2FC'}}>{price}</span>
                  <span style={{fontSize:13.5,color:'rgba(235,242,252,0.38)',marginLeft:4}}>{period}</span>
                </div>
                <p style={{fontSize:13.5,color:'rgba(235,242,252,0.48)',lineHeight:1.7,marginBottom:26}}>{desc}</p>
                <div style={{flex:1,marginBottom:28,display:'flex',flexDirection:'column',gap:11}}>
                  {features.map(f=>(
                    <div key={f} style={{display:'flex',gap:10,alignItems:'flex-start'}}>
                      <span style={{color:highlight?'#F5B830':'#12D4B4',fontSize:13,marginTop:1,flexShrink:0}}>✓</span>
                      <p style={{fontSize:13.5,color:'rgba(235,242,252,0.62)',lineHeight:1.5}}>{f}</p>
                    </div>
                  ))}
                </div>
                <Link href={href} className={highlight?'cta-main':'cta-ghost'} style={{display:'block',textAlign:'center',padding:'13px',borderRadius:10,fontWeight:700,fontSize:14,textDecoration:'none',border:highlight?'none':'1px solid rgba(255,255,255,0.1)',color:highlight?'#060C1A':'rgba(235,242,252,0.65)',transition:'all .2s'}}>
                  {cta} →
                </Link>
              </div>
            ))}
            ...
            {[
              ...
              <div key={label}>
                ...
                <div style={{display:'flex',flexDirection:'column',gap:10}}>
                  {links.map(l=>(
                    <a key={l} href="#" style={{fontSize:13.5,color:'rgba(235,242,252,0.42)',textDecoration:'none',transition:'color .15s'}}
                      onMouseEnter={e=>(e.currentTarget.style.color='#EBF2FC')}
                      onMouseLeave={e=>(e.currentTarget.style.color='rgba(235,242,252,0.42)')}>
                      {l}
                    </a>
                  ))}
                </div>
