...
async function uploadToR2(
  ...
  const R2_ACCOUNT = process.env.R2_ACCOUNT_ID!; // ← was CLOUDFLARE_ACCOUNT_ID
...
function parseSVGBlocks(text: string): {
  icon: string;
  wordmark: string;
  combination: string;
} {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!svgs.icon && !svgs.wordmark) {
      return NextResponse.json(
        {
          error: "Claude did not generate valid SVG output. Please try again.",
        },
        { status: 500 },
      );
    }
    ...
    if (svgs.icon)
      urls.icon_svg = await uploadToR2(
        svgs.icon,
        `${prefix}_icon.svg`,
        "image/svg+xml",
      );
    if (svgs.wordmark)
      urls.wordmark_svg = await uploadToR2(
        svgs.wordmark,
        `${prefix}_wordmark.svg`,
        "image/svg+xml",
      );
    if (svgs.combination)
      urls.combination_svg = await uploadToR2(
        svgs.combination,
        `${prefix}_combination.svg`,
        "image/svg+xml",
      );
    ...
    if (svgForPng) {
      ...
      urls.png_1500 = await uploadToR2(
        png1500,
        `${prefix}_1500.png`,
        "image/png",
      );
    ...
    return NextResponse.json(
      { error: err.message || "Logo generation failed" },
      { status: 500 },
    );
  }
}
