...
export async function POST(request: NextRequest) {
  ...
  // 4. Update last_login
  await adminAny.from('admin_profiles').update({ last_login: new Date().toISOString() }).eq('id', profile.id)
...
export async function DELETE(request: NextRequest) {
  ...
  if (session) {
    const admin = createAdminClient()
    const adminAny = admin as any
    await adminAny.from('admin_sessions').update({ is_active: false, logged_out_at: new Date().toISOString() }).eq('id', session.sessionId)
