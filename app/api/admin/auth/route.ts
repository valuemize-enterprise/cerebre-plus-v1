...
function isRateLimited(ip: string): boolean {
  ...
  if (!record || now - record.first > WINDOW_MS) {
    RATE_LIMIT.set(ip, { count: 1, first: now });
    return false;
  }
  record.count++;
...
export async function POST(request: NextRequest) {
  ...
  if (!email || !password)
    ...
    // 1. Sign in via Supabase Auth using a SEPARATE client instance
const authClient = createClient(
  process.env.NEXT_PUBLIC_SUPABASE_URL!,
  process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!, // anon key is correct here — this is just verifying credentials
  ...
  if (error) {
    console.error("admin_profiles query error", error);
    return NextResponse.json({ error: "Server error" }, { status: 500 });
  }

  if (!profile)
    return NextResponse.json(
      { error: "Not an admin account. Contact your super admin." },
      { status: 403 },
    );
  ...
  const { data: session, error: gg } = await admin
    ...
    if (gg) {
    console.error("admin_session", gg);
    return NextResponse.json({ error: "Server error" }, { status: 500 });
  }
