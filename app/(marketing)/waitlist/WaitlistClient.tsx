...
const SHEET = 'https://script.google.com/macros/s/AKfycbxAE-rmVxxh5wyjrXZ2irjWXJbwsDSxMACxkxCFxGJKDpjPaPsPMC7lPN6g6Dewmo6b/exec'
const CLAIMED = 247
...
export default function WaitlistClient() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const s = JSON.parse(localStorage.getItem('cp_wl') || '[]'); localStorage.setItem('cp_wl', JSON.stringify([...s, p]))
      await fetch(SHEET, { method: 'POST', mode: 'no-cors', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(p) })
    } catch (err) {
      console.error(err)
    } finally {
      setBusy(false)
      ...
      <div style={{ fontFamily: "'Syne',system-ui,sans-serif", background: C.void, color: C.text, WebkitFontSmoothing: 'antialiased', lineHeight: 1.7 }}>
        ...
        <section id="tools" style={sec(C.ink)}>
          <div style={wrap}>
            ...
            <div style={{ marginTop: 20, background: `${C.teal}08`, border: `1px dashed ${C.teal}28`, borderRadius: 14, padding: '20px 24px' }}>
              <div style={{ fontWeight: 700, color: C.bright, fontSize: 15, marginBottom: 6 }}>20+ More Tools Are Coming</div>
          ...
          <div style={wrap}>
            ...
            <div className="prg">
              {[
                ...
                { badge: '⚡ Most Popular', name: 'Cerebre Growth', amt: '₦35,000', period: '/month', strike: 'Regular price: ₦65,000/month', desc: 'The full marketing team experience. All tools, priority access, community.', features: ['250 Cerebre Coins/month', 'Everything in Starter, plus:', 'Priority access to all 20+ new tools', 'Tuesday live group onboarding', 'Nigerian Business Owners WhatsApp community', 'Priority WhatsApp support'], cta: 'Join Waitlist — Growth →', feat: true },
              ].map(p => (
        ...
        <section id="bonus" style={sec()}>
          <div style={wrap}>
            ...
            <div className="bg">
              {[
                { e: '⚡', t: 'Immediate access to every new tool as it launches', d: 'All 20+ tools coming to the platform — included in your subscription at no extra cost.' },
